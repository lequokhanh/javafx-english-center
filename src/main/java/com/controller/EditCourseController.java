...
public class EditCourseController {
    ...
    public static void show(Course course, CoursesController handle) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(
                Objects.requireNonNull(EditCourseController.class.getResource(Constants.FXML_EDIT_COURSE)));
        ...
        if (course == null) {
            try {
                ...
                controller.submit.setOnAction(e -> {
                    try {
                        if (controller.courseName.getText().isEmpty()
                                || controller.courseDescription.getText().isEmpty()
                                || controller.courseLevel.getValue() == null) {
                            ErrorController.show("Please fill all the fields");
                            return;
                        }
                        CourseService.InsertNewCourse(controller.courseID.getText(), controller.courseName.getText(),
            ...
            controller.submit.setOnAction(e -> {
                try {
                    if (controller.courseName.getText().isEmpty() || controller.courseDescription.getText().isEmpty()
                            || controller.courseLevel.getValue() == null) {
                        ErrorController.show("Please fill all the fields");
                        return;
                    }
                    CourseService.UpdateCourse(controller.course.getCourseID(), controller.courseName.getText(),
                            controller.courseDescription.getText(), controller.courseLevel.getValue());
                    handle.search("");
                    controller.close();
                } catch (IOException | SQLException ex) {
