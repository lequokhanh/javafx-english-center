...
public class ChapterService {
    ...
    public static ObservableList<Chapter> search(String courseID, String keyWord) throws SQLException, IOException {
        ...
        ResultSet result = db.select(String.format(
                "select * from chapter where course_id = '%s' and (upper(id) like '%%%s%%' or upper(name) like '%%%s%%' or upper(description) like '%%%s%%' or upper(category) like '%%%s%%') order by id asc",
                courseID, keyWord, keyWord, keyWord, keyWord));
    ...
    public static void InsertNewChapter(String id, String courseID, String name, String description, String category)
            throws SQLException {
        DBConnection db = new DBConnection();
        String query = String.format("INSERT INTO chapter VALUES ('%s', '%s', '%s', '%s', '%s')", id, courseID, name,
    ...
    public static void UpdateChapter(String id, String name, String description, String category) throws SQLException {
        ...
        String query = String.format(
                "UPDATE chapter SET name = '%s', description = '%s', category = '%s' WHERE id = '%s'", name,
